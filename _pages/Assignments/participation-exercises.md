...
8.  **[Tree-Walking Interpretation, Binding, and Scope]({{ site.baseurl }}/Assignments/ParticipationExercises/BindingAndScope)** - handed out for *Tree-Walking Interpretation*, and again on the *Binding and Scope* day as the Mystery Scoping Language exercise.  Evaluate a shadowing expression by hand under both lexical and dynamic scope, drawing the environment at each step, then deduce a hidden scoping rule by experiment.
