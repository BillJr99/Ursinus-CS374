...
info:
  coursenum: CS374
  purpose: "To get the Regular Expressions assignment off the ground with a partner by building a working test harness, your first tested patterns, and the finditer mini-lexer skeleton that the Regex and Lexer assignments both grow."
  tilt:
    ...
    criteria: "I grade this on a working harness with three passing tested patterns and a mini-lexer skeleton that tokenizes the worked example correctly, weighted 50/50 across the two parts.  The rubric below breaks it down in full."
  points: 100
  goals:
...
This **lab** is the on-ramp for the Regular Expressions assignment, which I hand out the same day.  In one sitting with a partner you'll build the two artifacts everything in that assignment grows from.  The first is the `check()` test harness with your first three passing patterns, and the assignment's Part 1 asks for ten.  The second is the `re.finditer` mini-lexer skeleton, which the assignment's Part 2 completes and the Lexer assignment later turns into a permanent pipeline component.  One sitting should do it.
