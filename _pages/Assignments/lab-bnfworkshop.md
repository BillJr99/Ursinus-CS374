...
> 2. Write `<symbol>` in EBNF, down to terminals.  One way in is to build it from smaller productions and then say `<symbol>` is any of them: a production for an ordinary name, which is one or more letters (digits are fine after the first character) followed by an optional `?` or `!`; a production for the operators, which matches the set `+ - * / < > = ` and the comparisons built from it; and so on for any other shape you want to allow.  Decide which characters may *start* a symbol and which may appear after that, and write those as two separate sets so that a leading digit is impossible.  The characters you must account for are the letters, the digits, and the extended set `+ - * / < > = ? !`.
