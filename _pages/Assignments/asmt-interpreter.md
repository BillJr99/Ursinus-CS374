...
This assignment completes your pipeline with a tree-walking evaluator: a program that walks the abstract syntax tree (AST) your parser built and runs each node.  Your language gets real scopes, real types, a REPL (a read-eval-print loop, the interactive prompt), and a semantics document.  This is the component your team project extends, and the semantics document matters as much as the code does.  Build in the scaffolded order below, since each part depends on the one before it.
...
You need Python 3.10 or newer plus your completed Lexer and Parser.  Copy `lexer.py`, `parser.py`, `ast_nodes.py`, and `token_spec.json` into the project directory and import them unchanged (note any bug fixes in your readme).  Create the deliverable files up front:
...
Confirm the pipeline is connected before you evaluate anything.  Run `python -c "from parser import parse; print(parse('print 1 + 2;'))"`; it should print a `Program` tree.
...
Build the dispatch skeleton before any evaluation logic, exactly as Step 1b's worked example asks:

1.  Copy the node dataclasses from Step 1a, reconciling them with your parser's existing nodes.
2.  Write the `Interpreter` class with an `eval_node` method that has one branch per node type.  Make each branch raise `NotImplementedError` for now.
3.  Check that the wiring works:
...
Then fill in only the `Num` branch (`return node.value`) and the `Print` branch, and run `print 42;` end to end through your lexer, parser, and evaluator.  Once one statement flows through the entire pipeline on day one, the rest of Part 2 is filling in branches one at a time.
...
A Python `dataclass` writes `__init__`, `__repr__`, and optional `__eq__` for you.  Clean node types make the evaluator's `isinstance` dispatch readable, and `__repr__` makes test failures self-documenting.
...
Write an `Interpreter` class with one `eval_node(node)` method.  The method dispatches on node type using `isinstance`: it checks which class the node belongs to and runs the matching branch.  Every node type listed above must have its own branch.  An `else` branch raises `InterpreterError("Unknown node type: ...")`.

**Worked example:** before you implement any evaluation logic, verify that `eval_node(Num(42))` raises `NotImplementedError` (or returns a placeholder), and that every node type reaches its own branch rather than the else.
...
An environment is the table that maps variable names to their current values.  Each block gets its own environment with a link to its parent, which is how nested scopes work.  The `Environment` class:
...
Verify that the **shadowing program** prints `51` then `2`:
...
When a property fails, Hypothesis shrinks it to the minimal offending program.  In your `readme.md`, report one invariant that caught a real bug (or a reasoned all-clear, with the three properties and the generator shown).
...
Include this **REPL session transcript** in your submission:
...
Your language's `define` statements and function definitions carry (or are extended to carry) type annotations: `let x: Num = 42;`, `fun f(a: Num, b: Str) -> Bool { ... }`.  The checker walks the AST once, maintaining a type environment that mirrors your `Environment` class, and verifies three things:
...
Five programs are provided whose outputs depend on your semantics decisions.  Run each, record the output, and confirm that it matches your SEMANTICS.md documentation.  If it does not match, fix either the code or the documentation; they must agree.
