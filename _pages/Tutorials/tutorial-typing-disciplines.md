...
This is the core companion for the **Type Systems** unit.  It pins down two axes that people constantly confuse, places real languages on them, and then looks at **gradual typing**, the discipline behind mypy and TypeScript that lets a single program be part-checked and part-unchecked.  It connects directly to the two type stories in your own pipeline: the **dynamic** typing your Interpreter enforces at runtime, and the **static** Hindley-Milner checker offered as a direction of that assignment.
...
4.  Weakly-typed C lets you reinterpret an `int`'s bits as a `float`.  Name one situation where that is useful and one where it is a catastrophe.
